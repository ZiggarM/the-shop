...
export const ProductPageComponent: FC<Props> = ({
  ...
  const productCreateUpdateHandler = async (
    ...
    const uploadFile = async (file: File) => {
      const uniqueId = uuid();
      const fileName = `product/product-${uniqueId}-${file.name}`; // Replace with your desired file name
    ...
    switch (intent) {
      ...
      case "update": {
        if (heroImageUrl && images.length > 0) {
          await updateProduct({
            category: Number(category),
            heroImage: heroImageUrl,
            imagesUrl: imageUrls,
    ...
    <main className="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8">
      <div className="container mx-auto p-4">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-2xl font-bold">Products Managment</h1>
          <Button
            onClick={() => {
              setCurrentProduct(null);
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-[100px]">Max Quantity</TableHead>
              <TableHead className="w-[100px]">Hero Image</TableHead>
              <TableHead className="w-[100px]">Product Images</TableHead>
